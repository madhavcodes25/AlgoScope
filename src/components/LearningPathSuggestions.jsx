...
import { difficultyColors } from './DifficultyBadge'
...
export default function LearningPathSuggestions({ className = '' }) {
  ...
  return (
    <div className={`bg-slate-900/50 border border-slate-700/60 rounded-xl px-5 py-4 flex flex-col gap-3 backdrop-blur-sm shadow-xl ${className}`}>
      <h3 className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-400/80">
        Suggested Next Steps
      </h3>
      <p className="text-xs text-slate-400 font-light leading-relaxed">
        Finished this {activeDifficulty} topic? Continue your learning journey with these recommended modules:
      </p>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-1">
        {suggestions.map((topic) => {
          const colorStyles = difficultyColors[topic.difficulty] || difficultyColors.Beginner
          return (
            <Link
              key={topic.name}
              to={topic.href}
            ...
            >
              <div className="flex flex-col min-w-0">
                ...
                <span className={`text-[9px] font-mono mt-0.5 ${colorStyles.text} uppercase tracking-wider`}>
                  {topic.difficulty}
                </span>
              </div>
